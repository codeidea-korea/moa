...
?>
			...
			<?php
				// 이미지 상단 출력
				$v_img_count = count($teacher['file']);
				if($v_img_count && $is_img_head) {
					echo '<div class="view-img">'.PHP_EOL;
					for ($i=0; $i<=count($teacher['file']); $i++) {
						if ($teacher['file'][$i]['view']) {
							echo get_view_thumbnail($teacher['file'][$i]['view']);
						}
					}
					echo '</div>'.PHP_EOL;
				}
			...
			<div itemprop="description" class="view-content">
				<?php echo get_view_thumbnail($teacher['content']); ?>
			</div>
				...
				<div id="sit_pvi">
					<div id="sit_pvi_big" >
						...
						for($i=1; $i<=10; $i++) {
							...
							// 큰이미지 썸네일
							$imgs = debs_thumbnail($it['it_img'.$i], $big_w, $big_h);
							$img = "<img src='/data/item/".$imgs['src']."' />";
							
							if($img) {
								// 썸네일
								$thumb = debs_thumbnail($it['it_img'.$i], 10, 10);
								$thumbnails[] = $thumb;
