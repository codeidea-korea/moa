...
<div class="gw_m_sns">
	<p class="gw_m_sns_con"><i class="fa fa-share-alt" aria-hidden="true"></i>SNS 공유</p>
	<ul id="bo_v_sns" class="<?php echo $bo_v_sns_class; ?>">
		...
		<?php if($config['cf_kakao_js_apikey']) { ?>
		<li><a href="javascript:kakaolink_send('<?php echo str_replace(array('%27', '\''), '', $sns_msg); ?>', '<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>');" class="sns_k"><img src="<?php echo G5_SNS_URL; ?>/icon/kakaotalk.png" alt="카카오톡으로 보내기" width="20"><span>카카오톡 공유</span></a></li>
		<?php } ?>
	</ul>
</div>
...
<script>
	// sns mobile 
	$(".gw_m_sns").click(function(){
		$("#bo_v_sns").toggleClass("on");
    });
</script>