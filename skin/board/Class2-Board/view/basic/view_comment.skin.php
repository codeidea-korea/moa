...
$( document ).ready(function() {
    $('#header_write').removeClass('dp_none')
});
</script>

<?php if($cmt_amt) { ?>
<section id="bo_vc" class="s_content">
	<?php
	for ($i=0; $i<$cmt_amt; $i++) {
	...
	<div class="com_other">
		<ul class="community">
			<li>
				<?php echo ($cmt_depth) ? ' <ul class="ment"> <li>' : ''; ?>
					<div>
						<div class="info">
							<div class="com_img">
								<?php 
									if($is_cmt_photo) { // 회원사진
										...
										$cmt_photo = ($cmt_photo_url) ? '<img src="'.$cmt_photo_url.'" alt="" class="media-object">' : '<img src="../images/default_img.svg" alt="">';
						...
						<div class="com_tarea">
							<p>
								...
								<?php echo $list[$i]['name'] ?><span><?php echo apms_date($list[$i]['date'], 'orangered', 'before');?></span></p>
	...
	<aside id="bo_vc_w">
		...
		<div class="chat_win">
			<div class="chat_area">
				...
				<div class="chat_send">
					<textarea style="width:100%;font-size:1.4rem;height:50px;line-height:50px;padding:0 10px;" tabindex="13" id="wr_content" name="wr_content" maxlength="10000" rows=5 class="form-control input-sm" title="내용"
					...
					<button onclick="apms_comment_submit();" onKeyDown="apms_comment_onKeyDown();" id="btn_submit">보내기</button>
				</div>
			</div>
		</div>
