...
<!-- 관리자 로긴 후 강의등록 및 강의일정 등록 -->
<?php if ($rss_href || $write_href) { ?>
<ul class="btn_bo_user">
	<?php if ($admin_href) { ?><li><a href="<?php echo "/bbs/write.php?bo_table=class&a=r" ?>" class="btn_b01">강의정보등록</a></li><?php } ?>
	<!--
	<?php if ($admin_href) { ?><li><a href="<?php echo "/bbs/board.php?bo_table=reserv" ?>" class="btn_b01">예약하기</a></li><?php } ?>
	<?php if ($admin_href) { ?><li><a href="<?php echo "/bbs/board.php?bo_table=aplylist" ?>" class="btn_b01">예약신청자 리스트</a></li><?php } ?>
	<?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02"><font color="#ffffff">강의일정 등록</font></a></li><?php } ?>
...
<table border="0" cellspacing="5" cellpadding="5" id="yearmonth">
	<tr>
		<td style="display:none"><a href="<?php echo $_SERVER['PHP_SELF']."?bo_table=".$bo_table."&opt=ship&"; ?><?php if ($month == 1) { $year_pre=$year-1; $month_pre=$month; } else {$year_pre=$year-1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url ?>/img/y_prev.gif" border="0" alt="<?php echo $year_pre ?>년"></a></td>

		<td><a href="<?php echo $_SERVER['PHP_SELF']."?bo_table=".$bo_table."&opt=ship&"; ?><?php if ($month == 1) { $year_pre=$year-1; $month_pre=12; } else {$year_pre=$year; $month_pre=$month-1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url ?>/img/m_prev_big.gif" border="0" alt="<?php echo $month_pre ?>월"></a></td>

		<td class="today">[강의일정] : <?php echo $year ?>  년 <?php echo $month ?>월</td>

		<td><a href="<?php echo $_SERVER['PHP_SELF']."?bo_table=".$bo_table."&opt=ship&"; ?><?php if ($month == 12) { $year_pre=$year+1; $month_pre=1; } else {$year_pre=$year; $month_pre=$month+1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url ?>/img/m_next_big.gif" border="0" alt="<?php echo $month_pre ?>월"></a></td>

		<td style="display:none"><a href="<?php echo $_SERVER['PHP_SELF']."?bo_table=".$bo_table."&opt=ship&"; ?><?php if ($month == 12) { $year_pre=$year+1; $month_pre=$month; } else {$year_pre=$year+1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url ?>/img/y_next.gif" border="0" alt="<?php echo $year_pre ?>년"></a></td>
	</tr>
</table>			
		

<div id="bo_list">
<table width="<?php echo $width ?>" bgcolor="#cfcfcf" border="1" cellspacing="1" cellpadding="5"  class="bo_list">
<thead>
...
while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
	...
	for ($i = $start_day ; $i <= $end_day;  $i++) {
		...
		$f_date = substr($row['wr_1'],0,4)."-".substr($row['wr_1'],4,2)."-".substr($row['wr_1'],6,2);
		...
		$html_day[$i].= "<br />#<img src='$board_skin_url/img/".$imgown.".gif' border=0 align=absmiddle /> <a href='".G5_BBS_URL."/board.php?bo_table={$bo_table}&year={$year}&month={$month}&wr_id={$row['wr_id']}&sc_no={$sc_no}&r_date={$f_date}' id='subject_".$j."' ".$showLayer.">".$row['wr_subject']."</a>".$list['icon_new'].$list['comment_cnt'];
			?>
		    <!-- 뷰 팝업레이어 -->
			...
			<?php
		//오늘 스케줄 구하기
		if ($row['wr_id'] != $sc_id && date('Ymd', strtotime($row['wr_1'])) <= date('Ymd') && date('Ymd', strtotime($row['wr_2']))) { 
			//>= date('Ymd')) {
