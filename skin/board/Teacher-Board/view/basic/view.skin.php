...
<section itemscope itemtype="http://schema.org/NewsArticle">
	<article itemprop="articleBody">
		...
		<?php
		// 프로파일 보기 
		$list = get_portfolio($view['mb_id']);
		//print_r2($list);

		
		echo "*** profile *** ***";
		$profile = $list['profile'];
		$procount = $profile['count'];
		//print_r2($list['profile']);
		for ($i = 0; $i < $procount; $i++) {
			echo " 순번 : ".$i."<BR>";
			echo " 이미지경로 : ".$profile[$i]['path']."/".$profile[$i]['file']."<BR>";
			echo " title : ".$profile[$i]['title']."<BR>";
			echo " role : ".$profile[$i]['role']."<BR>";
		}
		...
		for ($i = 0; $i < $hiscount; $i++) {
			echo " 순번 : ".$i."<BR>";
			echo " 유형 : ".($history[$i]['type'])?'기타경력':'메인경력'."<BR>";
			echo " 기간  : ".$history[$i]['asis']." ~ ".$history[$i]['tobe']."<BR>";
			echo " 내용 : ".$history[$i]['cootent']."<BR>";
