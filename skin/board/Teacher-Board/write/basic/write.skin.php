...
<div class="form-group">
	<label class="col-sm-2 control-label" for="wr_subject">선생님<strong class="sound_only">필수</strong></label>
	<div class="col-sm-10">
...
<div class="form-group">
	<label class="col-sm-2 control-label" for="wr_subject">선생님소개<strong class="sound_only">필수</strong></label>
	<div class="col-sm-10">
...
<?php } ?>
	...
	<div class="form-group">
		<label class="col-sm-2 control-label hidden-xs">상태</label>
		<div class="col-sm-3">
...
<?php for ($i=1; $is_link && $i<=G5_LINK_COUNT; $i++) { ?>
	<div class="form-group">
		<label class="col-sm-2 control-label" for="wr_link<?php echo $i ?>">포트폴리오 링크 #<?php echo $i ?></label>
		<div class="col-sm-10">
...
<?php if ($is_file) { ?>
	<div class="form-group">
		<label class="col-sm-2 control-label hidden-xs">포트폴리오파일 첨부</label>
		<div class="col-sm-10">
			<p class="form-control-static text-muted" style="padding:0px; padding-top:4px;">
				<span class="cursor" onclick="add_file();"><i class="fa fa-plus-circle fa-lg"></i> 포트폴리오 추가</span>
				&nbsp;
				<span class="cursor" onclick="del_file();"><i class="fa fa-times-circle fa-lg"></i> 포트폴리오 삭제</span>
			</p>
		</div>
	</div>
	...
	<script>
	var flen = 0;
	function add_file(delete_code,values) {
		...
		if (values) {
			...
			if (flen == 0) {
				objContent += "<div class='col-sm-10'><div class='form-group'><div class='input-group input-group-sm '><input type='text' class='form-control input-sm' name='pf_title[]' placeholder='메인 포트폴리오 제목 '><input type='text' class='form-control input-sm' name='pf_role[]' placeholder='역할'><span class='input-group-addon' style='width:100px'>메인소개사진 "+objNum+"</span><input type='file'  accept='image/*;capture=camera' class='form-control input-sm' name='bf_file[]' title='파일 용량 <?php echo $upload_max_filesize; ?> 이하만 업로드 가능'></div></div></div>";
			}
			else {
				objContent += "<div class='col-sm-10'><div class='form-group'><div class='input-group input-group-sm '><input type='text' class='form-control input-sm' name='pf_title[]' placeholder='포트폴리오 제목''><input type='text' class='form-control input-sm' name='pf_role[]' placeholder='역할'><span class='input-group-addon' style='width:100px'>포트폴리오 "+objNum+"</span><input type='file'  accept='image/*;capture=camera' class='form-control input-sm' name='bf_file[]' title='파일 용량 <?php echo $upload_max_filesize; ?> 이하만 업로드 가능'></div></div></div>";
			} 
		}
		if (delete_code) {
...
<?php } ?>
	<div class="form-group">
		<label class="col-sm-2 control-label hidden-xs">선생님 경력</label>
		<div class="col-sm-10">
			<p class="form-control-static text-muted" style="padding:0px; padding-top:4px;">
				<span class="cursor" onclick="add_history();"><i class="fa fa-plus-circle fa-lg"></i> 경력 추가</span>
				&nbsp;
				<span class="cursor" onclick="del_history();"><i class="fa fa-times-circle fa-lg"></i> 경력 삭제</span>
			</p>
		</div>
	</div>
	<div class="form-group" style="margin-bottom:0;">
	...
	<script>
	var hlen = 0;
	function add_history(delete_code,values) {
		var upload_count = <?php echo (int)$board['bo_upload_count']; ?>;
		/*if (upload_count && flen >= upload_count) {
			alert("이 게시판은 "+upload_count+"개 까지만 파일 업로드가 가능합니다.");
			return;
		...
		*/
		...
		if (values) 
			...
			objContent += "<div class='col-sm-10'><div class='form-group'><div class='input-group input-group-sm '><select name='ph_type[]' class='form-control'><option value='0'>주요경력</option><option value='1'>그밖의경력</option></select><input type='text' class='form-control input-sm' name='ph_asis[]' placeholder='시작일자 '>~<input type='text' class='form-control input-sm' name='ph_tobe[]' placeholder='종료일자 '><input type='text' class='form-control input-sm' name='ph_content[]' placeholder='경력내용'></div></div></div>";
		if (delete_code) {
			objContent += delete_code;
		} else {
			<?php if ($is_file_content) { ?>
	...
	<?php echo $history_script; //수정시에 필요한 스크립트?>
