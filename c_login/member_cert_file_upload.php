...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    $upload[$i]['file']     = $_FILES['bf_file']['name'];
    $upload[$i]['source']   = $_FILES['bf_file']['name'];
    $upload[$i]['filesize'] = $_FILES['bf_file']['size'];
    ...
    if (is_uploaded_file($tmp_file)) {
        $filesize = round($filesize / (1024*1024), 4) .'M';
        // 관리자가 아니면서 설정한 업로드 사이즈보다 크다면 건너뜀
        if (!$is_admin && $filesize > $upload_max_filesize) {
            $file_upload_msg .= '\"' . $filename . '\" 파일의 용량(' . number_format($filesize) . ' KB)이 게시판에 설정(' . number_format($upload_max_filesize) . ' MB)된 값보다 크므로 업로드 하지 않습니다.\\n';
            continue;
        }
            //=================================================================\
            // 090714
