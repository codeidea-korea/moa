...
for($inx = 0; $inx < count($tags); $inx++){
    ...
    $sql = " select count(deitem.wr_id) cnt 
            ...
            and deitem.wr_id = ".$tags['wr_id']." 
            and deitem.day < '".$startDay."'
            and DATE_ADD(deitem.day, INTERVAL 1 DAY); > '".$startDay."' ";
    $result = sql_query($sql);
    $row = sql_fetch_array($result);

    if($row['cnt'] >= 1) {
        // 모임 다음날 호스트 후기 요청 알림톡
        include_once(G5_LIB_PATH."/kakao_alimtalk.lib.php");
        {
            $replaceText = '';
            $reserve_type = 'NORMAL';
            $start_reserve_time = date('Y-m-d H:i:s');
            $reciver = '{"name":"'.$tags["mb_name"].'","mobile":"'.$tags["mb_hp"].'"}';
            sendBfAlimTalk(156, $replaceText, $reserve_type, $reciver, $start_reserve_time);
        }

        // 모임 다음날 게스트 후기 요청 알림톡
        $sql = " select a.*,b.mb_hp,b.mb_name
                from deb_class_aplyer a
                    left join g5_member b ON a.mb_id = b.mb_id
                where 1=1 
                and a.wr_id = ".$tags['wr_id']."
                and a.status = '예약확정'";
        $result = sql_query($sql);
