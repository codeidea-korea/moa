...
else if ($w == 'u')
{
    ...
    if (!$mv_no)
        alert("선택된 베너가 없습니다. ");
    $mb = getTableContents($mv_no,'itemvod','mv');
 ...
 $g5['title'] .= "";
$g5['title'] .= ' 강의 상세 공통항목  '.$html_title;
...
<form name="fmember" id="fmember" action="./itemvod_form_update.php" onsubmit="return fmember_submit(this);" method="post" enctype="multipart/form-data">
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr style="display:none">
        ...
        <td colspan="3">
            ...
            if (file_exists($icon_file)) {
                //$icon_url = G5_DATA_URL.'/itemvod/'.$mv_dir.'/'.$mb['mv_img_pc'];
    ...
    <tr style="display:none">
        ...
        <td colspan="3">
            ...
            <?php
            $mv_dir = 'mobile';
            //$icon_file = G5_DATA_PATH.'/itemvod/'.$mv_dir.'/'.$mb['mv_img_mo'];
            $icon_file = G5_PATH.$mb['mv_img_mo'];
            if (file_exists($icon_file)) {
                $icon_url = G5_DATA_URL.'/itemvod/'.$mv_dir.'/'.$mb['mv_img_mo'];
    ...
    <?php
     $mv_nm = array("1"=>"제목","2"=>"버튼이름","3"=>"버튼링크");
     for ($i=1; $i<=10; $i++) { ?>
    <tr>
        <th scope="row"><label for="mv_<?php echo $i ?>">
            <?php
            $cnt = count($mv_nm);
             if ($i <= $cnt){
                echo $mv_nm[$i];
...
<div class="btn_fixed_top">
    <a href="./itemvod_list.php?<?php echo $qstr ?>" class="btn btn_02">목록</a>
