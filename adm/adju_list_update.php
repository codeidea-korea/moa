...
if ($_POST['act_button'] == "선택수정") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        $mb = get_adju_basic($_POST['no'][$k]);

        if (!$mb['no']) {
            $msg .= $mb['no'].' : 정산자료가 존재하지 않습니다.\\n';
        }
        else {
            ...
            $sql = " update {$g5['adju_basic_table']}
    ...
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        if (!$mb['no']) {
            ...
            // 회원자료 삭제
            //member_delete($mb['no']);
            $sql =" delete from {$g5['adju_basic_table']} where no = '{$mb['no']}' ";
            echo $sql."<BR>";
			sql_query($sql, false);
            //exit;
