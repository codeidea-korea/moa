...
if ($w == "")
{
    ...
    if (!$it['ca_id'])
    {
        $sql = " select ca_id from {$g5['g5_shop_category_table']} where ca_id like '10%' order by ca_order, ca_id limit 1 ";
        $row = sql_fetch($sql);
        if (!$row['ca_id'])
            alert("등록된 카테고리가 없습니다. 우선 카테고리를 등록하여 주십시오.", './categorylist.php');
        $it['ca_id'] = $row['ca_id'];
    }
...
else if ($w == "u")
{
    ...
    if ($is_admin != 'super')
    {
        $sql = " select it_id from {$g5['g5_shop_item_table']} a, {$g5['g5_shop_category_table']} b
                  where  a.ca_id like '10%' and a.it_id = '$it_id'
                    and a.ca_id = b.ca_id
                    and b.ca_mb_id = '{$member['mb_id']}' ";
        $row = sql_fetch($sql);
        if (!$row['it_id'])
            alert("\'{$member['mb_id']}\' 님께서 수정 할 권한이 없는 모임입니다.");
    }
    ...
    if(!$it)
        alert('모임상품정보가 존재하지 않습니다.');

    if (!$ca_id)
        $ca_id = $it['ca_id'];

    $sql = " select * from {$g5['g5_shop_category_table']} where  ca_id like '10%' and ca_id = '$ca_id' ";
    $ca = sql_fetch($sql);
}
else
{
...
<li><a href="#anc_sitfrm_cate">모임카테고리</a></li>
<li><a href="#anc_sitfrm_ini">기본정보</a></li>
<li><a href="#anc_sitfrm_compact">요약정보</a></li>
<li><a href="#anc_sitfrm_cost">옵션항목</a></li>
<li><a href="#anc_sitfrm_img">모임이미지</a></li>
<li><a href="#anc_sitfrm_curriculum">관련커리큘럼</a></li>
<li><a href="#anc_sitfrm_relation">관련모임</a></li>
<li><a href="#anc_sitfrm_relation2">관련도서</a></li>
...
<form name="fitemmoa_form" action="./itemmoa_formupdate.php" method="post" enctype="MULTIPART/FORM-DATA" autocomplete="off" onsubmit="return fitemmoa_formcheck(this)">
...
<section id="anc_sitfrm_cate">
	...
	<div class="box-header">모임 기본정보</div>

    <div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>모임카테고리 입력</caption>
        <colgroup>
            <col class="grid_4">
            <col>
        </colgroup>
        <tbody>
        <tr>
            <th scope="row"><label for="ca_id">기본카테고리</label></th>
            <td style="display:">
                <?php if ($w == "") echo help("기본카테고리를 선택하면, 판매/재고/HTML사용/판매자 E-mail 등을, 선택한 카테고리의 기본값으로 설정합니다."); ?>
        ...
        <?php for ($i=2; $i<=2; $i++) { ?>
        <tr>
            <th scope="row"><label for="ca_id<?php echo $i; ?>"><?php echo $i; ?>차 카테고리</label></th>
            <td>
                <?php echo help($i.'차 카테고리는 기본 카테고리의 하위 카테고리 개념이 아니므로 기본 카테고리 선택시 해당 모임이 포함될 최하위 카테고리만 선택하시면 됩니다.'); ?>
...
<section id="anc_sitfrm_skin">
    <div class="box-header">스킨설정</div>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p>모임상세보기에서 사용할 스킨을 설정합니다.</p>
    </div>
...
<section id="anc_sitfrm_ini">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
                <?php if ($w == '') { // 추가 ?>
                    ...
                    <?php echo help("모임의 코드는 10자리 숫자로 자동생성합니다. <b>직접 모임코드를 입력할 수도 있습니다.</b>\n모임코드는 영문자, 숫자, - 만 입력 가능합니다."); ?>
                ...
                <?php } else { ?>
                    ...
                    <a href="<?php echo G5_SHOP_URL; ?>/item.php?it_id=<?php echo $it_id; ?>" class="btn_frmline">모임확인</a>
                    <a href="<?php echo G5_ADMIN_URL; ?>/shop_admin/itemuselist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">사용후기</a>
                    <a href="<?php echo G5_ADMIN_URL; ?>/shop_admin/itemqalist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">모임문의</a>
                <?php } ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_order">우선순위</label></th>
            <td>
        ...
        <tr>
            <th scope="row">모임유형</th>
            <td>
                <?php echo help("메인화면에 유형별로 출력할때 사용합니다.\n이곳에 체크하게되면 모임리스트에서 유형별로 정렬할때 체크된 모임이 가장 먼저 출력됩니다."); ?>
                ...
                <label for="it_type3">신모임 <img src="<?php echo G5_SHOP_URL; ?>/img/icon_new.gif" alt=""></label>
        ...
        <tr>
            <th scope="row"><label for="it_name">모임상품명</label></th>
            <td colspan="2">
        ...
        <tr>
            <th scope="row"><label for="it_maker">제작사</label></th>
            <td>
                <?php echo help("입력하지 않으면 모임상세페이지에 출력하지 않습니다."); ?>
            ...
            <td class="td_grpset">
                ...
                <tr>
            <th scope="row"><label for="it_cust_price">정상금액</label></th>
            <td>
                <?php echo help("입력하지 않으면 모임상품상세페이지에 출력하지 않습니다."); ?>
        ...
        <tr>
            <th scope="row"><label for="it_price">할인금액</label></th>
            <td>
...
<tr style="display:">
            <th scope="row"><label for="it_origin">모임시간</label></th>
            <td>
                <?php echo help("숫자로만 입력하세요(일수입니다.)."); ?>
                <input type="text" name="it_origin" value="<?php echo get_text($it['it_origin']); ?>" id="it_origin" class="frm_input" size="10"> DAYS
            </td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row"><label for="it_basic">특별혜택</label></th>
            <td>
                <?php echo help("모임명 하단에 모임에 대한 추가적인 설명이 필요한 경우에 입력합니다. HTML 입력도 가능합니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("모임 금액 대신 전화문의로 표시됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("설정에 체크하시면 쿠폰 생성 때 모임 검색 결과에 노출되지 않습니다."); ?>
        ...
        <tr>
            ...
            <td>
                <?php echo help("운영자와 실제 판매자가 다른 경우 실제 판매자의 e-mail을 입력하면, 모임상품 주문 시점을 기준으로 실제 판매자에게도 주문서를 발송합니다."); ?>
...
<section id="anc_sitfrm_compact">
    <div class="box-header">모임상품요약정보</div>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p><strong>전자상거래 등에서의 상품 등의 정보제공에 관한 고시</strong>에 따라 상품 특성 등을 양식에 따라 입력할 수 있습니다.</p>
    </div>
    ...
    <div id="sit_compact">
        ...
        <select id="it_info_gubun" name="it_info_gubun">
            ...
            <?php
            if(!$it['it_info_gubun']) $it['it_info_gubun'] = 'book';
...
<section id="anc_sitfrm_cost">
    <div class="box-header">옵션항목</div>
    <?php echo $pg_anchor; ?>

    <div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>옵션항목입력</caption>
        <colgroup>
            <col class="grid_4">
            <col>
            <col class="grid_3">
        ...
        <tr style="display:none">
            <th scope="row"><label for="it_soldout">모임상품품절</label></th>
            <td>
                <?php echo help("잠시 판매를 중단하거나 재고가 없을 경우에 체크해 놓으면 품절모임상품으로 표시됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td colspan="2">
                <?php echo help("상품이 품절인 경우에 체크해 놓으면 모임상품상세보기에서 고객이 재입고SMS 알림을 신청할 수 있게 됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("<b>주문관리에서 모임상품별 상태 변경에 따라 자동으로 재고를 가감합니다.</b> 재고는 규격/색상별이 아닌, 모임상품별로만 관리됩니다.<br>재고수량을 0으로 설정하시면 품절모임상품으로 표시됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("모임상품의 재고가 통보수량보다 작을 때 쇼핑몰관리 메인화면의 재고현황에 재고부족 모임상품으로 표시됩니다.<br>옵션이 있는 모임상품은 개별 옵션의 통보수량이 적용됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("모임상품 구매시 최소 구매 수량을 설정합니다."); ?>
        ...
        <tr >
            ...
            <td>
                <?php echo help("모임상품 구매시 최대 구매 수량을 설정합니다."); ?>
        ...
        <tr style="display:none">
            <th scope="row"><label for="it_notax">모임상품과세 유형</label></th>
            <td>
                <?php echo help("모임상품의 과세유형(과세, 비과세)을 설정합니다."); ?>
        ...
        ?>
        <tr>
            <th scope="row">모임상품추가옵션</th>
            <td colspan="2">
                <div id="sit_supply_frm" class="sit_option tbl_frm01">
                    <?php echo help('옵션항목은 콤마(,) 로 구분하여 여러개를 입력할 수 있습니다. 스마트폰을 예로 들어 [추가1 : 추가구성모임상품 , 추가1 항목 : 액정보호필름,케이스,충전기]<br><strong>옵션명과 옵션항목에 따옴표(\', ")는 입력할 수 없습니다.</strong>'); ?>
                    <table>
                    <caption>모임상품추가옵션 입력</caption>
                    <colgroup>
                        <col class="grid_4">
                        <col>
                    </colgroup>
...
<section id="anc_sitfrm_sendcost"  style="display:none">
    <div class="box-header">배송비</div>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p>쇼핑몰설정 &gt; 배송비유형 설정보다 <strong>개별모임상품 배송비설정이 우선</strong> 적용됩니다.</p>
    </div>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
            ...
            <tr id="sc_con_qty">
                ...
                <td>
                    <?php echo help("모임상품의 주문 수량에 따라 배송비가 부과됩니다. 예를 들어 기본배송비가 3,000원 수량을 3으로 설정했을 경우 모임상품의 주문수량이 5개이면 6,000원 배송비가 부과됩니다."); ?>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="it_img<?php echo $i; ?>">
                <?php if ($i == 1) {echo "대표";}?>이미지 <?php echo $i; ?></label></th>
            <td>
...
<section id="anc_sitfrm_curriculum" class="srel"  style="display:">
    <div class="box-header">관련커리큘럼</div>
    <?php echo $pg_anchor; ?>

    <div class="compare_wrap">
        <section class="compare_left">
            <h3>등록된 전체커리큘럼 목록</h3>
            <div id="curriculum_list" class="srel_list srel_noneimg">
                <?php
                $sql = " select cc_id, cc_name from {$g5['lec_curriculum_table']} order by cc_id desc ";
                $result = sql_query($sql);
                ...
                ?>
                    <li>
                        ...
                        <div class="list_item"><?php echo get_text($row['cc_name']); ?></div>
                        <div class="list_item_btn"><button type="button" class="add_curriculum btn_frmline">추가</button></div>
                    </li>
                <?php
                }
                ...
                if($g > 0)
                    ...
                    echo '<p>등록된 커리큘럼이 없습니다.</p>';
                ?>
            </div>
            <script>
            $(function() {
                $(document).on("click", "#curriculum_list .add_curriculum", function() {
                    // 이미 등록된 커리큘럼인지 체크
                    var $li = $(this).closest("li");
                    var cc_id = $li.find("input:hidden").val();
                    var cc_id2;
                    var dup = false;
                    $("#reg_curriculum_list input[name='cc_id[]']").each(function() {
                        cc_id2 = $(this).val();
                        if(cc_id == cc_id2) {
                            dup = true;
                            return false;
                    ...
                    if(dup) {
                        alert("이미 선택된 커리큘럼입니다.");
                        return false;
                    }

                    var cont = "<li>"+$li.html().replace("add_curriculum", "del_curriculum").replace("추가", "삭제")+"</li>";
                    var count = $("#reg_curriculum_list li").size();

                    if(count > 0) {
                        $("#reg_curriculum_list li:last").after(cont);
                    } else {
                        $("#reg_curriculum_list").html("<ul>"+cont+"</ul>");
                    }
                });

                $(document).on("click", "#reg_curriculum_list .del_curriculum", function() {
                    ...
                    var count = $("#reg_curriculum_list li").size();
                    if(count < 1)
                        $("#reg_curriculum_list").html("<p>선택된 커리큘럼가 없습니다.</p>");
                });
            });
            </script>
        </section>
        ...
        <section class="compare_right">
            <h3>선택된 관련커리큘럼 목록</h3>
            <div id="reg_curriculum_list" class="srel_sel srel_noneimg">
                <?php
                $str = "";
                $comma = "";
                $sql = " select b.cc_id, b.cc_name
                           from {$g5['item_curriculum_table']} a
                           left join {$g5['lec_curriculum_table']} b on (a.cc_id=b.cc_id)
                          where  a.it_id = '$it_id'
                          order by b.cc_id desc ";
                $result = sql_query($sql);
                for ($g=0; $row=sql_fetch_array($result); $g++) {
                    $str .= $comma . $row['cc_id'];
                    $comma = ",";
                ...
                ?>
                    <li>
                        <input type="hidden" name="cc_id[]" value="<?php echo $row['cc_id']; ?>">
                        <div class="list_item"><?php echo get_text($row['cc_name']); ?></div>
                        <div class="list_item_btn"><button type="button" class="del_curriculum btn_frmline">삭제</button></div>
                    </li>
                <?php
                }
                ...
                if($g > 0)
                    ...
                    echo '<p>선택된 커리큘럼가 없습니다.</p>';
                ?>
            </div>
            <input type="hidden" name="cc_list" value="<?php echo $str; ?>">
        </section>
    </div>
...
<section id="anc_sitfrm_relation" class="srel">
    <div class="box-header">관련모임상품</div>
    <?php echo $pg_anchor; ?>

    <div class="local_desc02 local_desc">
        <p>
            등록된 전체모임상품 목록에서 모임상품분류를 선택하면 해당 모임상품 리스트가 연이어 나타납니다.<br>
            모임상품리스트에서 관련 모임상품으로 추가하시면 선택된 관련모임상품 목록에 <strong>함께</strong> 추가됩니다.<br>
            예를 들어, A 모임상품에 B 모임상품을 관련모임상품으로 등록하면 B 모임상품에도 A 모임상품이 관련모임상품으로 자동 추가되며, <strong>확인 버튼을 누르셔야 정상 반영됩니다.</strong>
        </p>
    </div>
    ...
    <div class="compare_wrap">
        <section class="compare_left">
            <h3>등록된 전체모임상품 목록</h3>
            <label for="sch_relation" class="sound_only">모임상품분류</label>
            <span class="srel_pad">
                <select id="sch_relation">
                    <option value=''>분류별 모임상품</option>
                    <?php
                        $sql = " select * from {$g5['g5_shop_category_table']} ";
                        if ($is_admin != 'super')
                            $sql .= " where   ca_id like '10%'  and ca_mb_id = '{$member['mb_id']}' ";
                ...
                <label for="sch_name" class="sound_only">모임상품명</label>
            ...
            <div id="relation" class="srel_list">
                <p>모임상품의 분류를 선택하시거나 모임상품명을 입력하신 후 검색하여 주십시오.</p>
            </div>
            <script>
            $(function() {
                $("#btn_search_item").click(function() {
                    ...
                    /*if(ca_id == "" && it_name == "") {
                        $relation.html("<p>모임상품의 분류를 선택하시거나 모임상품명을 입력하신 후 검색하여 주십시오.</p>");
                        return false;
                    }
                    */

                    $("#relation").load(
                        "./itemmoa_formrelation.php",
                ...
                $(document).on("click", "#reg_relation .del_item", function() {
                    ...
                    var count = $("#reg_relation li").size();
                    if(count < 1)
                        $("#reg_relation").html("<p>선택된 모임상품이 없습니다.</p>");
                });
            });
            </script>
        </section>
        ...
        <section class="compare_right">
            ...
            <div id="reg_relation" class="srel_sel">
                ...
                $sql = " select b.ca_id, b.it_id, b.it_name, b.it_price
                           ...
                           left join {$g5['g5_shop_item_table']} b on ( b.ca_id like '10%' and a.it_id2=b.it_id)
                          where  a.it_id = '$it_id'
                          order by ir_no asc ";
                $result = sql_query($sql);
                //echo $sql."<BR>";
...
<section id="anc_sitfrm_relation2" class="srel">
    <div class="box-header">관련도서상품</div>
    <?php echo $pg_anchor; ?>

    <div class="local_desc02 local_desc">
        <p>
            등록된 전체도서상품 목록에서 도서상품분류를 선택하면 해당 도서상품 리스트가 연이어 나타납니다.<br>
            도서상품리스트에서 관련 도서상품으로 추가하시면 선택된 관련도서상품 목록에 <strong>함께</strong> 추가됩니다.<br>
            예를 들어, A 도서상품에 B 도서상품을 관련도서상품으로 등록하면 B 도서상품에도 A 도서상품이 관련도서상품으로 자동 추가되며, <strong>확인 버튼을 누르셔야 정상 반영됩니다.</strong>
        </p>
    </div>
    ...
    <div class="compare_wrap">
        <section class="compare_left">
            <h3>등록된 전체도서상품 목록</h3>
            <label for="sch_relation" class="sound_only">도서상품분류</label>
            <span class="srel_pad">
                <select id="sch_relation_book">
                    <option value=''>분류별 도서상품</option>
                    <?php
                        $sql = " select * from {$g5['g5_shop_category_table']} ";
                        if ($is_admin != 'super')
                            $sql .= " where   ca_id like '20%'  and ca_mb_id = '{$member['mb_id']}' ";
                        else
                            $sql .= " where   ca_id like '20%'   ";
                        $sql .= " order by ca_order, ca_id ";
                        $result = sql_query($sql);
                        for ($i=0; $row=sql_fetch_array($result); $i++)
                        {
                ...
                <label for="sch_name" class="sound_only">도서상품명</label>
                <input type="text" name="sch_name_book" id="sch_name_book" class="frm_input" size="15">
                <button type="button" id="btn_search_item_book" class="btn_frmline">검색</button>
            </span>
            <div id="relation_book" class="srel_list">
                <p>도서상품의 분류를 선택하시거나 도서상품명을 입력하신 후 검색하여 주십시오.</p>
            </div>
            <script>
            $(function() {
                $("#btn_search_item_book").click(function() {
                    var ca_id = $("#sch_relation_book").val();
                    var it_name = $.trim($("#sch_name_book").val());
                    var $relation = $("#relation_book");
                    ...
                    */
                    ...
                    $("#relation_book").load(
                        "./itembook_formrelation.php",
                        { it_id: "<?php echo $it_id; ?>", ca_id: ca_id, it_name: it_name }
                    );
                });

                $(document).on("click", "#relation_book .add_item", function() {
                    // 이미 등록된 도서상품인지 체크
                    var $li = $(this).closest("li");
                    var it_id = $li.find("input:hidden").val();
                    var it_id2;
                    var dup = false;
                    $("#reg_relation_book input[name='re_it_id_book[]']").each(function() {
                        it_id2 = $(this).val();
                        //alert(it_id2);
                        if(it_id == it_id2) {
                            dup = true;
                    ...
                    if(dup) {
                        alert("이미 선택된 도서상품입니다.");
                        return false;
                    }

                    var cont = "<li>"+$li.html().replace("add_item", "del_item").replace("추가", "삭제")+"</li>";
                    var count = $("#reg_relation_book li").size();

                    if(count > 0) {
                        $("#reg_relation_book li:last").after(cont);
                    } else {
                        $("#reg_relation_book").html("<ul>"+cont+"</ul>");
                    }

                    $li.remove();
                });

                $(document).on("click", "#reg_relation_book .del_item", function() {
                    if(!confirm("도서상품을 삭제하시겠습니까?"))
                        return false;
                    ...
                    var count = $("#reg_relation_book li").size();
                    if(count < 1)
                        $("#reg_relation_book").html("<p>선택된 도서상품이 없습니다.</p>");
                });
            });
            </script>
        </section>
        ...
        <section class="compare_right">
            <h3>선택된 관련도서상품 목록</h3>
            <span class="srel_pad"></span>
            <div id="reg_relation_book" class="srel_sel">
                <?php
                $str = array();
                $sql = " select b.ca_id, b.it_id, b.it_name, b.it_price
                           from {$g5['g5_shop_item_relation_table']} a
                           join {$g5['g5_shop_item_table']} b on ( b.ca_id like '20%' and a.it_id2=b.it_id)
                          where  a.it_id = '$it_id'
                          order by ir_no asc ";
                $result = sql_query($sql);
                //echo $sql."<BR>";
                for($g=0; $row=sql_fetch_array($result); $g++)
                {
                    ...
                    ?>
                    <li>
                        <input type="hidden" name="re_it_id_book[]"  value="<?php echo $row['it_id']; ?>">
                        <div class="list_item"><?php echo $it_name; ?></div>
                        <div class="list_item_btn"><button type="button" class="del_item btn_frmline">삭제</button></div>
                    </li>
                <?php
                ...
                if($g > 0)
                    ...
                    echo '<p>선택된 도서상품이 없습니다.</p>';
                ?>
            </div>
            <input type="hidden" name="it_list_book" value="<?php echo $str; ?>">
        </section>
...
<section id="anc_sitfrm_event" class="srel"  style="display:none">
    <div class="box-header">관련이벤트</div>
    <?php echo $pg_anchor; ?>
        ...
        <section class="compare_right">
            ...
            <div id="reg_event_list" class="srel_sel srel_noneimg">
                ...
                $sql = " select b.ev_id, b.ev_subject
                           ...
                           left join {$g5['g5_shop_event_table']} b on (b. ca_id like '10%' and a.ev_id=b.ev_id)
                          where  a.ca_id like '10%' and a.it_id = '$it_id'
...
<section id="anc_sitfrm_optional">
    <div class="box-header">상세설명설정</div>
    <?php echo $pg_anchor; ?>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row">모임상품상단내용</th>
            <td><?php echo help("모임상품상세설명 페이지 상단에 출력하는 HTML 내용입니다."); ?><?php echo editor_html('it_head_html', get_text($it['it_head_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row">모임상품하단내용</th>
            <td><?php echo help("모임상품상세설명 페이지 하단에 출력하는 HTML 내용입니다."); ?><?php echo editor_html('it_tail_html', get_text($it['it_tail_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr  style="display:none">
            <th scope="row">모바일 모임상품상단내용</th>
            <td><?php echo help("모바일 모임상품상세설명 페이지 상단에 출력하는 HTML 내용입니다."); ?><?php // echo editor_html('it_mobile_head_html', get_text($it['it_mobile_head_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr  style="display:none">
            <th scope="row">모바일 모임상품하단내용</th>
            <td><?php echo help("모바일 모임상품상세설명 페이지 하단에 출력하는 HTML 내용입니다."); ?><?php //echo editor_html('it_mobile_tail_html', get_text($it['it_mobile_tail_html'], 0)); ?></td>
            <td class="td_grpset">
...
<section id="anc_sitfrm_extra">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
                <?php echo help("모임상품을 처음 입력(등록)한 시간입니다."); ?>
                <?php echo $it['it_time']; ?>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="2">
                <?php echo help("모임상품을 최종 수정한 시간입니다."); ?>
...
<div class="btn_fixed_top">
    <a href="./itemmoa_list.php?<?php echo $qstr; ?>" class="btn btn_02">목록</a>
    <a href="<?php echo G5_SHOP_URL ;?>/item.php?it_id=<?php echo $it_id ;?>" class="btn_02  btn">모임상품보기</a>
...
function codedupcheck(id)
{
    ...
    if(it_id.length > 0) {
        alert("모임상품코드는 영문자, 숫자, -, _ 만 사용할 수 있습니다.");
        return false;
    }
    ...
    $.post(
        ...
        function(data) {
            if(data.name) {
                ...
                document.fitemmoa_form.codedup.value = '';
            }
        }, "json"
    );
}

function fitemmoa_formcheck(f)
{
    if (!f.ca_id.value) {
        alert("기본분류를 선택하십시오.");
        f.ca_id.focus();
    ...
    // 관련도서상품처리
    var item_book = new Array();
    var re_item_book = it_id_book = "";
    ...
    $("#reg_relation_book input[name='re_it_id_book[]']").each(function() {
        ...
        item_book.push(it_id_book);
    });

    if(item_book.length > 0)
        re_item_book = item_book.join();

    $("input[name=it_list_book]").val(re_item_book);
    // 이벤트처리
    var evnt = new Array();
    var ev = ev_id = "";
...
categorychange(document.fitemmoa_form);
</script>
