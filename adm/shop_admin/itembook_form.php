...
if ($w == "")
{
    ...
    if (!$it['ca_id'])
    {
        $sql = " select ca_id from {$g5['g5_shop_category_table']} where ca_id like '20%' order by ca_order, ca_id limit 1 ";
        $row = sql_fetch($sql);
        if (!$row['ca_id'])
...
else if ($w == "u")
{
    ...
    if ($is_admin != 'super')
    {
        $sql = " select it_id from {$g5['g5_shop_item_table']} a, {$g5['g5_shop_category_table']} b
                  where  a.ca_id like '20%' and a.it_id = '$it_id'
                    and a.ca_id = b.ca_id
                    and b.ca_mb_id = '{$member['mb_id']}' ";
        $row = sql_fetch($sql);
        if (!$row['it_id'])
            alert("\'{$member['mb_id']}\' 님께서 수정 할 권한이 없는 도서입니다.");
    }
    ...
    if(!$it)
        alert('도서상품정보가 존재하지 않습니다.');

    if (!$ca_id)
        $ca_id = $it['ca_id'];

    $sql = " select * from {$g5['g5_shop_category_table']} where  ca_id like '20%' and ca_id = '$ca_id' ";
    $ca = sql_fetch($sql);
}
else
{
...
<li><a href="#anc_sitfrm_cate">도서카테고리</a></li>
...
<li><a href="#anc_sitfrm_img">도서이미지</a></li>
<li><a href="#anc_sitfrm_relation">관련강의</a></li>
...
<form name="fitembook_form" action="./itembook_formupdate.php" method="post" enctype="MULTIPART/FORM-DATA" autocomplete="off" onsubmit="return fitembook_formcheck(this)">
...
<section id="anc_sitfrm_cate">
    <h2 class="h2_frm">도서카테고리</h2>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p>기본카테고리는 반드시 선택하셔야 합니다. 하나의 도서에 최대 2개의 다른 카테고리를 지정할 수 있습니다.</p>
    </div>

    <div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>도서카테고리 입력</caption>
        <colgroup>
            <col class="grid_4">
            <col>
        </colgroup>
        ...
        <tr>
            ...
            <td>
                <?php echo help($i.'차 카테고리는 기본 카테고리의 하위 카테고리 개념이 아니므로 기본 카테고리 선택시 해당 도서이(가) 포함될 최하위 카테고리만 선택하시면 됩니다.'); ?>
...
<section id="anc_sitfrm_ini">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
                <?php if ($w == '') { // 추가 ?>
                    ...
                    <?php echo help("도서의 코드는 10자리 숫자로 자동생성합니다. <b>직접 도서코드를 입력할 수도 있습니다.</b>\n도서코드는 영문자, 숫자, - 만 입력 가능합니다."); ?>
                ...
                <?php } else { ?>
                    ...
                    <a href="<?php echo G5_SHOP_URL; ?>/item.php?it_id=<?php echo $it_id; ?>" class="btn_frmline">도서확인</a>
                    <a href="<?php echo G5_ADMIN_URL; ?>/shop_admin/itemuselist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">사용후기</a>
                    <a href="<?php echo G5_ADMIN_URL; ?>/shop_admin/itemqalist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">도서문의</a>
                <?php } ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <th scope="row">도서유형</th>
            <td>
                <?php echo help("메인화면에 유형별로 출력할때 사용합니다.\n이곳에 체크하게되면 도서리스트에서 유형별로 정렬할때 체크된 도서이 가장 먼저 출력됩니다."); ?>
                ...
                <label for="it_type3">신도서 <img src="<?php echo G5_SHOP_URL; ?>/img/icon_new.gif" alt=""></label>
        ...
        <tr>
            <th scope="row"><label for="it_name">도서상품명</label></th>
            <td colspan="2">
        ...
        <tr>
            ...
            <td>
                <?php echo help("입력하지 않으면 도서상세페이지에 출력하지 않습니다."); ?>
            ...
            <td>
                <?php echo help("입력하지 않으면 도서상품상세페이지에 출력하지 않습니다."); ?>
        ...
        <tr style="display:none">
            <th scope="row"><label for="it_origin">도서시간</label></th>
            <td>
        ...
        <tr>
            ...
            <td>
                <?php echo help("도서명 하단에 도서에 대한 추가적인 설명이 필요한 경우에 입력합니다. HTML 입력도 가능합니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("도서 금액 대신 전화문의로 표시됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("설정에 체크하시면 쿠폰 생성 때 도서 검색 결과에 노출되지 않습니다."); ?>
        ...
        <tr>
            ...
            <td>
                <?php echo help("운영자와 실제 판매자가 다른 경우 실제 판매자의 e-mail을 입력하면, 도서상품 주문 시점을 기준으로 실제 판매자에게도 주문서를 발송합니다."); ?>
...
<section id="anc_sitfrm_cost">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr style="display:">
            <th scope="row"><label for="it_soldout">도서상품품절</label></th>
            <td>
                <?php echo help("잠시 판매를 중단하거나 재고가 없을 경우에 체크해 놓으면 품절도서상품으로 표시됩니다."); ?>
        ...
        <tr style="display:">
            ...
            <td colspan="2">
                <?php echo help("상품이 품절인 경우에 체크해 놓으면 도서상품상세보기에서 고객이 재입고SMS 알림을 신청할 수 있게 됩니다."); ?>
        ...
        <tr style="display:">
            ...
            <td>
                <?php echo help("<b>주문관리에서 도서상품별 상태 변경에 따라 자동으로 재고를 가감합니다.</b> 재고는 규격/색상별이 아닌, 도서상품별로만 관리됩니다.<br>재고수량을 0으로 설정하시면 품절도서상품으로 표시됩니다."); ?>
        ...
        <tr style="display:">
            ...
            <td>
                <?php echo help("도서상품의 재고가 통보수량보다 작을 때 쇼핑몰관리 메인화면의 재고현황에 재고부족 도서상품으로 표시됩니다.<br>옵션이 있는 도서상품은 개별 옵션의 통보수량이 적용됩니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("도서상품 구매시 최소 구매 수량을 설정합니다."); ?>
        ...
        <tr style="display:none">
            ...
            <td>
                <?php echo help("도서상품 구매시 최대 구매 수량을 설정합니다."); ?>
        ...
        <tr style="display:none">
            <th scope="row"><label for="it_notax">도서상품과세 유형</label></th>
            <td>
                <?php echo help("도서상품의 과세유형(과세, 비과세)을 설정합니다."); ?>
        ...
        ?>
        <tr>
            <th scope="row">도서상품추가옵션</th>
            <td colspan="2">
                <div id="sit_supply_frm" class="sit_option tbl_frm01">
                    <?php echo help('옵션항목은 콤마(,) 로 구분하여 여러개를 입력할 수 있습니다. 스마트폰을 예로 들어 [추가1 : 추가구성도서상품 , 추가1 항목 : 액정보호필름,케이스,충전기]<br><strong>옵션명과 옵션항목에 따옴표(\', ")는 입력할 수 없습니다.</strong>'); ?>
                    <table>
                    <caption>도서상품추가옵션 입력</caption>
                    <colgroup>
                        <col class="grid_4">
                        <col>
                    </colgroup>
...
<section id="anc_sitfrm_sendcost"  style="display:">
    <h2 class="h2_frm">배송비</h2>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p>쇼핑몰설정 &gt; 배송비유형 설정보다 <strong>개별도서상품 배송비설정이 우선</strong> 적용됩니다.</p>
    </div>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
            ...
            <tr id="sc_con_qty">
                ...
                <td>
                    <?php echo help("도서상품의 주문 수량에 따라 배송비가 부과됩니다. 예를 들어 기본배송비가 3,000원 수량을 3으로 설정했을 경우 도서상품의 주문수량이 5개이면 6,000원 배송비가 부과됩니다."); ?>
...
<section id="anc_sitfrm_curriculum" class="srel"  style="display:none;">
    <h2 class="h2_frm">관련커리큘럼</h2>
    <?php echo $pg_anchor; ?>
...
<section id="anc_sitfrm_relation" class="srel">
    <h2 class="h2_frm">관련강의상품</h2>
    <?php echo $pg_anchor; ?>

    <div class="local_desc02 local_desc">
        <p>
            등록된 전체강의상품 목록에서 강의상품분류를 선택하면 해당 강의상품 리스트가 연이어 나타납니다.<br>
            강의상품리스트에서 관련 강의상품으로 추가하시면 선택된 관련강의상품 목록에 <strong>함께</strong> 추가됩니다.<br>
            예를 들어, A 강의상품에 B 강의상품을 관련강의상품으로 등록하면 B 강의상품에도 A 강의상품이 관련강의상품으로 자동 추가되며, <strong>확인 버튼을 누르셔야 정상 반영됩니다.</strong>
        </p>
    </div>
    ...
    <div class="compare_wrap">
        <section class="compare_left">
            <h3>등록된 전체강의상품 목록</h3>
            <label for="sch_relation" class="sound_only">강의상품분류</label>
            <span class="srel_pad">
                <select id="sch_relation">
                    <option value=''>분류별 강의상품</option>
                    <?php
                        $sql = " select * from {$g5['g5_shop_category_table']} ";
                        if ($is_admin != 'super')
                            $sql .= " where   ca_id like '20%'  and ca_mb_id = '{$member['mb_id']}' ";
                ...
                <label for="sch_name" class="sound_only">강의상품명</label>
            ...
            <div id="relation" class="srel_list">
                <p>강의상품의 분류를 선택하시거나 강의상품명을 입력하신 후 검색하여 주십시오.</p>
            </div>
            <script>
            $(function() {
                $("#btn_search_item").click(function() {
                    ...
                    /*if(ca_id == "" && it_name == "") {
                        $relation.html("<p>강의상품의 분류를 선택하시거나 강의상품명을 입력하신 후 검색하여 주십시오.</p>");
                        return false;
                    }
                    */
                ...
                $(document).on("click", "#reg_relation .del_item", function() {
                    ...
                    var count = $("#reg_relation li").size();
                    if(count < 1)
                        $("#reg_relation").html("<p>선택된 강의상품이 없습니다.</p>");
                });
            });
            </script>
        </section>
        ...
        <section class="compare_right">
            ...
            <div id="reg_relation" class="srel_sel">
                ...
                $sql = " select b.ca_id, b.it_id, b.it_name, b.it_price
                           ...
                           left join {$g5['g5_shop_item_table']} b on ( b.ca_id like '20%' and a.it_id2=b.it_id)
                          where  a.it_id = '$it_id'
                          order by ir_no asc ";
                $result = sql_query($sql);
                //echo $sql."<BR>";
        ...
        <section class="compare_left">
            ...
            $(function() {
                $("#btn_search_item_book").click(function() {
                    ...
                    /*if(ca_id == "" && it_name == "") {
                        $relation.html("<p>도서상품의 분류를 선택하시거나 도서상품명을 입력하신 후 검색하여 주십시오.</p>");
                        return false;
                    }
                    */
        ...
        <section class="compare_right">
            ...
            <div id="reg_event_list" class="srel_sel srel_noneimg">
                ...
                $sql = " select b.ev_id, b.ev_subject
                           ...
                           left join {$g5['g5_shop_event_table']} b on (b. ca_id like '20%' and a.ev_id=b.ev_id)
                          where  a.ca_id like '20%' and a.it_id = '$it_id'
...
<section id="anc_sitfrm_optional">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row">도서상품상단내용</th>
            <td><?php echo help("도서상품상세설명 페이지 상단에 출력하는 HTML 내용입니다."); ?><?php echo editor_html('it_head_html', get_text($it['it_head_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row">도서상품하단내용</th>
            <td><?php echo help("도서상품상세설명 페이지 하단에 출력하는 HTML 내용입니다."); ?><?php echo editor_html('it_tail_html', get_text($it['it_tail_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr  style="display:none">
            <th scope="row">모바일 도서상품상단내용</th>
            <td><?php echo help("모바일 도서상품상세설명 페이지 상단에 출력하는 HTML 내용입니다."); ?><?php // echo editor_html('it_mobile_head_html', get_text($it['it_mobile_head_html'], 0)); ?></td>
            <td class="td_grpset">
        ...
        <tr  style="display:none">
            <th scope="row">모바일 도서상품하단내용</th>
            <td><?php echo help("모바일 도서상품상세설명 페이지 하단에 출력하는 HTML 내용입니다."); ?><?php //echo editor_html('it_mobile_tail_html', get_text($it['it_mobile_tail_html'], 0)); ?></td>
            <td class="td_grpset">
...
<section id="anc_sitfrm_extra">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
                <?php echo help("도서상품을 처음 입력(등록)한 시간입니다."); ?>
                <?php echo $it['it_time']; ?>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="2">
                <?php echo help("도서상품을 최종 수정한 시간입니다."); ?>
...
<div class="btn_fixed_top">
    <a href="./itembook_list.php?<?php echo $qstr; ?>" class="btn btn_02">목록</a>
    <a href="<?php echo G5_SHOP_URL ;?>/item.php?it_id=<?php echo $it_id ;?>" class="btn_02  btn">도서상품보기</a>
...
function codedupcheck(id)
{
    ...
    if(it_id.length > 0) {
        alert("도서상품코드는 영문자, 숫자, -, _ 만 사용할 수 있습니다.");
        return false;
    }
    ...
    $.post(
        ...
        function(data) {
            if(data.name) {
                ...
                document.fitembook_form.codedup.value = '';
            }
        }, "json"
    );
}

function fitembook_formcheck(f)
{
    if (!f.ca_id.value) {
        alert("기본분류를 선택하십시오.");
        f.ca_id.focus();
...
categorychange(document.fitembook_form);
</script>
