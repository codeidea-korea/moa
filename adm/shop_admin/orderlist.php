...
if(! preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $sch_startdt) ) $sch_startdt = '';
if(! preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $sch_enddt) ) $sch_enddt = '';
...
if ($sch_startdt && $sch_enddt) {
    $where[] = " od_time between '$sch_startdt 00:00:00' and '$sch_enddt 23:59:59' ";
}

if ($where) {
    $sql_search = ' where a.od_id = b.od_id and b.it_id = c.it_id and '.implode(' and ', $where);
}
...
$qstr1 = "od_status=".urlencode($od_status)."&amp;od_settle_case=".urlencode($od_settle_case)."&amp;od_misu=$od_misu&amp;od_cancel_price=$od_cancel_price&amp;od_refund_price=$od_refund_price&amp;od_receipt_point=$od_receipt_point&amp;od_coupon=$od_coupon&amp;sch_startdt=$sch_startdt&amp;sch_enddt=$sch_enddt&amp;sel_field=$sel_field&amp;search=$search&amp;save_search=$search";
...
<form name="frmorderlist" class="local_sch01 local_sch no ne">
...
<form class="local_sch03 local_sch n one">
<div>
    <strong>주문상태</strong>
...
<div class="tbl-basic fs13 outline odd line-nth-1">
    ...
    <tr>
        ...
        <th scope="col" id="th_classid">class ID</th>
        <th scope="col" id="th_odrid">사용자ID</th>
        <th scope="col" id="th_odrer">사용자이름</th>
        <th scope="col" id="th_odrer">모임명</th>
        ...
        <th scope="col" id="odrdate">결제일</th>
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td headers="th_ordnum" class="td_classid2" >
        ...
        <td headers="th_odrer" class="td_name"><?php echo $row['od_name']; //$mb_nick; ?></td>
        <td headers="th_odrer" class="td_name"><?php echo $row['moa_name']; //$mb_nick; ?></td>
        ...
        <td  class="td_num_right"><?php echo number_format($row['pointprice']); ?></td>
        ...
        <td  class="td_mng td_mng_s">
            <a href="./orderform.php?od_id=<?php echo $row['od_id']; ?>&amp;<?php echo $qstr; ?>" class="mng_mod btn btn_02"><span class="sound_only"><?php echo $row['od_id']; ?> </span>상세/수정</a>
        </td>
...
<script>
$(function(){
    //$("#sch_startdt, #sch_enddt").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true, yearRange: "c-99:c+99", maxDate: "+0d" });
...
function set_date(today)
{
    ...
    ?>
    if (today == "오늘") {
        document.getElementById("sch_startdt").value = "<?php echo G5_TIME_YMD; ?>";
        document.getElementById("sch_enddt").value = "<?php echo G5_TIME_YMD; ?>";
    } else if (today == "어제") {
        document.getElementById("sch_startdt").value = "<?php echo date('Y-m-d', G5_SERVER_TIME - 86400); ?>";
        document.getElementById("sch_enddt").value = "<?php echo date('Y-m-d', G5_SERVER_TIME - 86400); ?>";
    } else if (today == "이번주") {
        document.getElementById("sch_startdt").value = "<?php echo date('Y-m-d', strtotime('-'.$date_term.' days', G5_SERVER_TIME)); ?>";
        document.getElementById("sch_enddt").value = "<?php echo date('Y-m-d', G5_SERVER_TIME); ?>";
    } else if (today == "이번달") {
        document.getElementById("sch_startdt").value = "<?php echo date('Y-m-01', G5_SERVER_TIME); ?>";
        document.getElementById("sch_enddt").value = "<?php echo date('Y-m-d', G5_SERVER_TIME); ?>";
    } else if (today == "지난주") {
        document.getElementById("sch_startdt").value = "<?php echo date('Y-m-d', strtotime('-'.$week_term.' days', G5_SERVER_TIME)); ?>";
        document.getElementById("sch_enddt").value = "<?php echo date('Y-m-d', strtotime('-'.($week_term - 6).' days', G5_SERVER_TIME)); ?>";
    } else if (today == "지난달") {
        document.getElementById("sch_startdt").value = "<?php echo date('Y-m-01', strtotime('-1 Month', $last_term)); ?>";
        document.getElementById("sch_enddt").value = "<?php echo date('Y-m-t', strtotime('-1 Month', $last_term)); ?>";
    } else if (today == "전체") {
        document.getElementById("sch_startdt").value = "";
        document.getElementById("sch_enddt").value = "";
    }
}
</script>
