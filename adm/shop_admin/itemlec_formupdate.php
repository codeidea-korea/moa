...
if($w == "u") {
    $sql = " select it_img1, it_img2, it_img3, it_img4, it_img5, it_img6, it_img7, it_img8, it_img9, it_img10
                ...
                where  ca_id like '10%' and it_id = '$it_id' ";
    $file = sql_fetch($sql);
...
$it_mobile_head_html = $it_head_html;
$it_mobile_tail_html = $it_tail_html;
...
if ($w == "" || $w == "u")
{
    ...
    // 관련 커리큘럼 등록
    $cc_id = explode(",", $cc_list);
    $sql = " delete from  {$g5['item_curriculum_table']}
                        where it_id = '$it_id' ";
    sql_query($sql, false);   

    for ($i=0; $i<count($cc_id); $i++)
    {
        if (trim($cc_id[$i]))
        {
            $sql = " insert into {$g5['item_curriculum_table']}
                        set cc_id = '$cc_id[$i]',
                            it_id = '$it_id',
                            ic_datetime = now() ";
            sql_query($sql, false);
    ...
    //                    where it_id = '$it_id' or it_id2 = '$it_id'";
    //sql_query($sql, false);   
    ...
    //sql_query($sql, false);   
     // 관련도서상품 등록
    //echo $it_list_book."<BR>";
    $it_id_book2 = explode(",", $it_list_book);
    for ($i=0; $i<count($it_id_book2); $i++)
    {
        if (trim($it_id_book2[$i]))
        {
            $sql = " insert into {$g5['g5_shop_item_relation_table']}
                        set it_id  = '$it_id',
                            it_id2 = '$it_id_book2[$i]',
                            ir_no = '$i' ";
            sql_query($sql, false);
            echo $sql."<BR>";
            // 관련상품의 반대로도 등록
            $sql = " insert into {$g5['g5_shop_item_relation_table']}
                        set it_id  = '$it_id_book2[$i]',
                            it_id2 = '$it_id',
                            ir_no = '$i' ";
            sql_query($sql, false);
            echo $sql."<BR>";
...
<script>
    if (confirm("계속 입력하시겠습니까?"))
        //location.href = "<?php echo "./itemlec_form.php?it_id=$it_id&amp;sort1=$sort1&amp;sort2=$sort2&amp;sel_ca_id=$sel_ca_id&amp;sel_field=$sel_field&amp;search=$search&amp;page=$page"?>";
        location.href = "<?php echo "./itemlec_form.php?".str_replace('&amp;', '&', $qstr); ?>";
    else
        location.href = "<?php echo "./itemlec_list.php?".str_replace('&amp;', '&', $qstr); ?>";
</script>
