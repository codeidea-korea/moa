...
<div class="tbl_frm01 tbl_wrap">
    ...
    <?php } ?>
    <tr>
        <th scope="row"><label for="ev_subject">이벤트 제목</label></th>
        <td>
            <input type="text" name="ev_subject" value="<?php echo htmlspecialchars2($ev['ev_subject']); ?>" id="ev_subject" required class="required frm_input"  size="60">
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="ev_mimg">대표 이미지</label></th>
        <td>
            <div class="fileContainer">									
				<label class="upload-btn"><input type="file" name="ev_mimg" class="preview">이미지 업로드</label>
				<?php	
				$mimg_str = "";
				$mimg = G5_DATA_PATH.'/event/'.$ev['ev_id'].'_m';
				if (file_exists($mimg)) {
					...
					if($size[0] && $size[0] > 750)
						...
						$width = $size[0];					
					$mimg_str = '<img src="'.G5_DATA_URL.'/event/'.$ev['ev_id'].'_m" width="'.$width.'" alt="">';
					$mimg_str .= '<label class="checkbox-wrap"><input type="checkbox" name="ev_mimg_del" value="1" id="ev_mimg_del"><span></span></label>';
				}
				?>
				<div class="upImg-preview"><?=$mimg_str?></div>
			</div>
        </td>
    </tr>
	<tr>
    ...
    <tr>
        ...
        <td>
            <textarea name="" class="" style="height:260px" placeholder="이벤트 내용을 입력해주세요."></textarea>
        </td>
    </tr>
    </tbody>
    </table>
