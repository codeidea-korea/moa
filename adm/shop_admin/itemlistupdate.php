...
if ($_POST['act_button'] == "선택수정") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++) {
		...
		$p_pt_pick = is_array($_POST['pt_pick']) ? strip_tags($_POST['pt_pick'][$k]) : '';
		$p_pt_plan = is_array($_POST['pt_plan']) ? strip_tags($_POST['pt_plan'][$k]) : '';
        ...
        $sql = "update {$g5['g5_shop_item_table']}
                   set ca_id          = '".sql_real_escape_string($p_ca_id)."',
					   ...
					   pt_plan		  = '".sql_real_escape_string($p_pt_plan)."',
					   pt_pick		  = '".sql_real_escape_string($p_pt_pick)."',
