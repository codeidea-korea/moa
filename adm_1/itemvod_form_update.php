...
/*
    $files = procUpload($_FILES['mv_img_pc'], 'itemvod', 'pc', true, $thumbwidth=100, $thumbheight=50, $delyn=false );
    $mv_img_pc = $files['name'];
    $mv_img_pc_thumb = $files['thumb'];

    $files = procUpload($_FILES['mv_img_mo'], 'itemvod', 'mobile', true, $thumbwidth=100, $thumbheight=50, $delyn=false );
    $mv_img_mo = $files['name'];
    $mv_img_mo_thumb = $files['thumb'];
*/
//echo $mb_id."<br>".$mv_no;exit;    
...
if ($w == '')
{
    ...
    if ($mb['mv_no'])
        alert('이미 존재하는 VOD 입니다.\\n '.$mb['mv_btn_text'].'\\n');

    
    $sql = " insert into {$g5['itemvod_table']} set mv_datetime = '".G5_TIME_YMDHIS."', {$sql_file} {$sql_common} ";
    sql_query($sql);
    //echo $sql."<BR>";
    $mv_no = sql_insert_id();
    //echo "mv_no = ".$mv_no."<BR>";
...
goto_url('./itemvod_form.php?'.$qstr.'&amp;w=u&amp;mv_no='.$mv_no, false);
?>