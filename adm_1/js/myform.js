...
$(document).ready(function(){
	...
	//checkbox
	$('.myContainer input[type="checkbox"]').each(function() {
	...
	//radio
	$('.myContainer input[type="radio"]').each(function() {
		var wrap = $(this).parent('label'),
			span = $(this).next('span');
		var label = typeof $(this).attr('data-label') !== typeof undefined && $(this).attr('data-label') !== '' ? $(this).attr('data-label') : '';
	...
	//input label
	$('.myContainer input:not([type="checkbox"]):not([type="radio"]):not([type="file"]):not([type="range"])').each(function() {
		var label = typeof $(this).attr('data-label') !== typeof undefined && $(this).attr('data-label') !== '' ? $(this).attr('data-label') : '',
	...
	//date
	$('.myContainer input.datepicker').each(function() {
	...
	$('.myContainer .inp-wrap').each(function() {
	...
	//date
	$('.myContainer input.search-date').each(function() {
		$(this).datepicker({
			language: 'ko-KR',
			autoHide: true,
			//autoPick: true,
	...
	$(".myContainer input.phone").bind("keyup", function(event) {
	...
	$(".myContainer input.price").bind("keyup", function(event) {
		$(this).val(numberFormat($(this).val().replace(/[^0-9]/g,"")));
	});

	//PX or % 단위 자동변경
	$('.myContainer input.percent').bind("keyup", function(event) {
	...
	$(".myContainer textarea.autoSize").bind("keypress", function(event) {
		textareaResize(this);
	});
	$(".myContainer textarea.autoSize").bind("keyup", function(event) {
		textareaResize(this);
	});

	$('.myContainer textarea.label').each(function() {
	...
	// 업로드 이미지 미리보기
	$('.myContainer input[type="file"].img').each(function(index) {
		var inp = $(this);
		var upload = $(this)[0];
