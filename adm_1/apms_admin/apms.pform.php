...
this.form.pt_leave.value=this.value; } else { this.form.pt_leave.value=this.form.pt_leave.defaultValue; }">
	...
	<tr>
        ...
        <td>
			<label><input type="checkbox" name="pt_partner" value="1"<?php echo ($mb['pt_partner'] == "1") ? ' checked' : '';?>> 호스트(셀러)</label>
			&nbsp;
	...
	<td colspan="4" style="padding-left:0px;padding-top:30px;">
		<h2 class="h2_frm" style="margin:0px;padding:0px;">
			정산정보 
			<span class="apms-helper"> - 호스트(셀러) 수수료 미등록시 '기본설정'의 기본 수수료가 자동적용되며, 추천인(마케터) 적립율은 각 상품에 따라 다릅니다.</span>
		</h2>
	</td>
	</tr>
	<tr>
        <th scope="row">일반상품 수수료</th>
        <td>부가세를 포함한 총판매금액에서 수수료 <input type="text" name="pt_commission_1" value="<?php echo $mb['pt_commission_1'] ?>" class="numeric sit_camt frm_input" size="4"> % 를 공제후 호스트(셀러)에게 적립</td>
        <th scope="row">일반상품 인센티브</th>
        <td>정산금액의 <input type="text" name="pt_incentive_1" value="<?php echo $mb['pt_incentive_1'] ?>" class="numeric sit_camt frm_input" size="4"> % 를 호스트(셀러)에게 추가 적립</td>
    </tr>
	<tr>
        <th scope="row">컨텐츠상품 수수료</th>
        <td>부가세를 포함한 총판매금액에서 수수료 <input type="text" name="pt_commission_2" value="<?php echo $mb['pt_commission_2'] ?>" class="numeric sit_camt frm_input" size="4"> % 를 공제후 호스트(셀러)에게 적립</td>
        <th scope="row">컨텐츠상품 인센티브</th>
        <td>정산금액의 <input type="text" name="pt_incentive_2" value="<?php echo $mb['pt_incentive_2'] ?>" class="numeric sit_camt frm_input" size="4"> % 를 호스트(셀러)에게 추가 적립</td>
    </tr>
	<tr>
        <th scope="row">상품 등록비</th>
        <td><input type="text" name="pt_point" value="<?php echo $mb['pt_point'] ?>" class="frm_input sit_camt" size="13"> 점 (분류 등록비와 합산, 호스트(셀러) 대상)</td>
        <th scope="row">일일 등록제한</th>
        <td><input type="text" name="pt_limit" value="<?php echo $mb['pt_limit'] ?>" class="frm_input sit_camt" size="4"> 개 상품만 호스트(셀러)는 하루동안 상품 등록 가능</td>
	</tr>
	<tr>
