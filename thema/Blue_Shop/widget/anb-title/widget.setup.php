...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		<td align="center">스타이더 스타일</td>
		<td>
			<select name="wset[slide_style]">
				<option value=""<?php echo get_selected('', $wset['slide_style']);?>>일 반</option>
				<option value="clear"<?php echo get_selected('clear', $wset['slide_style']);?>>클리어</option>
			</select>
			 클리어 스타일로 사용이 가능합니다.
		</td>
	</tr>
	...
	<?php for ($i=1; $i <= $wset['slider']; $i++) { 
			if(!$wset['baner'.$i]){
				$rowspan = 6;
			}else if($wset['baner'.$i] == '2'){
				$rowspan = 10;
			}else if($wset['baner'.$i] == '3'){
				$rowspan = 12;
			}else{
				$rowspan = 8;
			}
	?>
		...
		<tr>
			<td align="center"><b>서브배너</b></td>
			<td>
				<!--<input type="text" name="wset[baner<?php echo $i;?>]" value="<?php echo $wset['baner'.$i]; ?>" class="frm_input" size="4"> 개 - 입력후 저장하시면 배너분활이 가능합니다.-->
				<select name="wset[baner<?php echo $i;?>]">
					<option value=""<?php echo get_selected('',$wset['baner'.$i]);?>>사용안함</option>
					<option value="1"<?php echo get_selected('1', $wset['baner'.$i]);?>>전체배너</option>
					<option value="2"<?php echo get_selected('2', $wset['baner'.$i]);?>>2 배너</option>
					<option value="3"<?php echo get_selected('3', $wset['baner'.$i]);?>>3 배너</option>
				</select>
				선택후 저장하시면 배너분활이 가능합니다.
			</td>			
		</tr>
		<!--두개일때 세개일때 분환해주자 이미지와 링크-->	
		<?php for($s=1,$no=1; $s <= $wset['baner'.$i]; $s++,$no++){ ?>
			<?php // if($i != 1){ ;?>			
				<tr>
					<td align="center" >배너이미지 <?php echo $no; ?></td>
					<td>						
						<input type="text" name="wset[img<?php echo $i.'-'.$no;?>]" value="<?php echo ($wset['img'.$i.'-'.$no]);?>" id="img<?php echo $i.'-'.$no;?>" size="36" class="frm_input"> 
						<a href="<?php echo G5_BBS_URL;?>/widget.image.php?fid=img<?php echo $i.'-'.$no;?>" class="btn_frmline win_scrap">이미지선택</a>
					</td>
				</tr>
				<tr>
					<td align="center">배너링크 <?php echo $no; ?></td>
					<td>
						<?php echo help('URL(http://...)을 입력해야 하며, 미입력시 링크가 걸리지 않습니다.');?>
						<input type="text" name="wset[link<?php echo $i.'-'.$no;?>]" value="<?php echo $wset['link'.$i.'-'.$no]; ?>" size="40" class="frm_input" placeholder="http://...">
						&nbsp;
						타켓
						<input type="text" name="wset[target<?php echo $i.'-'.$no;?>]" value="<?php echo $wset['target'.$i.'-'.$no]; ?>" size="8" class="frm_input">
					</td>
				</tr>
			<?php // } ?>
		<?php } ?>
