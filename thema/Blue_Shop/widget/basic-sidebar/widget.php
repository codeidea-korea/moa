...
// 상단라인 컬러 - red, blue, green, orangered, black, orange, yellow, blue, violet, deepblue, crimson..
$line_top = 'blue';

// 메뉴 새글 아이콘 및 컬러 - red, blue, green, orangered, black, orange, yellow, blue, violet, deepblue, crimson..
		...
		<div class="sidebar-common">
			...
			<div class="btn-group btn-group-justified" role="group">
				<?php if($is_member) { ?>
					...
					<?php if($member['admin']) { ?>
						<a href="<?php echo G5_ADMIN_URL;?>" class="btn btn-blue btn-sm">관리자</a>
					<?php } ?>
					<?php if($member['partner']) { ?>
						<a href="<?php echo $at_href['myshop'];?>" class="btn btn-blue btn-sm">마이샵</a>
					<?php } ?>
					<a href="<?php echo $at_href['logout'];?>" class="btn btn-blue btn-sm">나가기</a>
				<?php } else { ?>
					<a href="#" onclick="sidebar_open('sidebar-user'); return false;" class="btn btn-blue btn-sm">로그인</a>
					<a href="<?php echo $at_href['reg'];?>" class="btn btn-blue btn-sm">회원가입</a>
					<a href="<?php echo $at_href['lost'];?>" class="win_password_lost btn btn-blue btn-sm">정보찾기</a>
				<?php } ?>
			</div>
