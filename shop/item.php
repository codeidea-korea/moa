...
$data = getWrInIt($it['it_id']);
$item_use = sql_query(" select * from g5_shop_item_use where is_confirm = 1 and it_id = '{$it_id}' order by it_id desc ");
$i = 0;
$total_score = 0;
$item = array();
while ($row = sql_fetch_array($item_use)) {
    $total_score += $row['is_score'];
    $item[$i]['is_score'] = $row['is_score'];
    $item[$i]['is_time'] = $row['is_time'];
    $item[$i]['is_name'] = $row['is_name'];
    $item[$i]['is_content'] = $row['is_content'];
