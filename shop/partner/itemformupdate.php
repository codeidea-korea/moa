...
if(!$is_auth) {
	...
	if($uplimit) { // 제한값이 있으면...
		$row = sql_fetch(" select count(*) as cnt from {$g5['g5_shop_item_table']} where pt_id = '{$member['mb_id']}' and date_format(it_time, '%Y-%c-%d') = '".G5_TIME_YMD."' and (ca_id = '{$uplimit_ca}' or ca_id2 = '{$uplimit_ca}' or ca_id3 = '{$uplimit_ca}') ");
