...
if($config['cf_editor'] && $is_dhtml_editor_use && $member['mb_level'] >= $board['bo_html_level']) {
    ...
    if ( $w == 'u' && (! $is_member || ! $is_admin || $write['mb_id'] !== $member['mb_id']) ){
        // kisa 취약점 제보 xss 필터 적용
        $content2 = get_text(html_purifier($write['moa_curriculum']), 0);
    }

    if(is_file(G5_EDITOR_PATH.'/'.$config['cf_editor'].'/autosave.editor.js'))
        $editor_content_js = '<script src="'.G5_EDITOR_URL.'/'.$config['cf_editor'].'/autosave.editor.js"></script>'.PHP_EOL;
}
$editor_html2 = editor_html('moa_curriculum', $content2, $is_dhtml_editor);



//$editor_js = '';
$editor_js .= get_editor_js('moa_curicculum', $is_dhtml_editor);
$editor_js .= chk_editor_js('moa_curicculum', $is_dhtml_editor);
...
///shop/partner/bbs/write.php?bo_table=class
...
$skin_path = G5_PATH."/shop/partner/skin/Basic";
