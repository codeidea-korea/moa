...
if ($a=="r") {
	$board_skin_url = G5_SKIN_URL."/board/LecSchedule";
	$board_skin_path = G5_SKIN_PATH."/board/LecSchedule";
...
//echo $qstr."<BR>";
?>
<link rel="stylesheet" href="<?php echo get_url($board_skin_url.'/style.css')?>" media="screen">
...
<section class="board-list<?php echo (G5_IS_MOBILE) ? ' font-14' : '';?>">
	...
	<div class="list-wrap">
		<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post" role="form" class="form">
			...
			<input type="hidden" name="moa_onoff" value="<?php echo $moa_onoff ?>">
			<input type="hidden" name="moa_area1" value="<?php echo $moa_area1 ?>">
			<input type="hidden" name="moa_area2" value="<?php echo $moa_area2 ?>">
			...
			?>
			<div class="list-btn-set none">
				<span class="openner"></span>
				<div class="btnContainer">
					<?php
					if($member['mb_level'] > 2 && $write_href) echo '<a role="button" href="'.$write_href.'" class="btn btn-write">모임등록</a>';
					if(false && $member['mb_level'] > 2) echo '<a role="button" href="'.$write_href.'&a=r" class="btn btn-write-calendar">일정등록</a>';
					echo '<a role="button" href="'.$write_href.'&a=r" class="btn btn-write-calendar">일정등록</a>';
					if($list_href) echo '<a role="button" href="'.$list_href.'" class="btn btn-list">목록</a>';
					if($member['mb_level'] > 2) echo '<a role="button" href="/bbs/board.php?bo_table=class&a=r" class="btn btn-list-calendar">일정목록</a>';		
					?>
				</div>
				<div class="cover"></div>
			</div>
				...
				<div class="form-group pull-right">
					<div class="btn-group dropup" role="group">
						...
						<?php if ($member['mb_level'] > 2) { ?>
							<a role="button" href="/bbs/board.php?bo_table=class&a=r" class="btn btn-<?php echo $btn1;?> btn-sm"><i class="fa fa-bars"></i> 일정목록</a>
						<?php } ?>
						<?php if ($member['mb_level'] > 2 && $write_href ) { ?><a role="button" href="<?php echo $write_href ?>" class="btn btn-<?php echo $btn2;?> btn-sm"><i class="fa fa-pencil"></i> 모임등록</a><?php } ?>
						<?php if (false && $member['mb_level'] > 2) { ?><a role="button" href="<?php echo $write_href ?>&a=r" class="btn btn-<?php echo $btn2;?> btn-sm"><i class="fa fa-pencil"></i> 일정등록</a><?php } ?>
					</div>
				</div>
			...
			<div class="list-page">
				<ul class="pagination">
					...
					echo apms_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&amp;page=');
					//if($next_part_href) echo '<li><a href="'.$next_part_href.'">다음검색</a></li>';
					?>
				</ul>
			</div>
		...
		<div class="modal fade none" id="searchModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
