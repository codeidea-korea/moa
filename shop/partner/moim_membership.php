...
if($wr_id == '') {
    $query = "select distinct a.*, m.mb_name, m.mb_sex, m.mb_birth, m.mb_hp, m.job_group, m.company_name, m.mb_id as uid, i.it_name as it_name 
        ...
        left join (select wr_id, it_id, min(day) as first_day from deb_class_item group by wr_id, it_id) as deb on i.it_id = deb.it_id and a.wr_id = deb.wr_id ";
    $query .= "WHERE a.it_id != '' and c.mb_id = '" . $member['mb_id'] . "' ". $exculusiveOutofTimeMoimWhenAdmin . " order by a.idx desc";
} else {
    $query = "SELECT distinct a.*, m.mb_name, m.mb_sex, m.mb_birth, m.mb_hp, m.job_group, m.company_name, m.mb_id as uid, i.it_name as it_name FROM deb_class_aplyer a LEFT JOIN g5_member m ON m.mb_id=a.mb_id 
        LEFT JOIN g5_write_class c ON a.wr_id = c.wr_id  left join g5_shop_item i on i.it_2 = a.wr_id 
        left join (select wr_id, it_id, min(day) as first_day from deb_class_item group by wr_id, it_id) as deb on i.it_id = deb.it_id and a.wr_id = deb.wr_id ";
