...
if ($bo_table != "aplylist")
if ($w == 'u' || $w == 'r') {
    $wr = get_write($write_table, $wr_id);
    if (!$wr['wr_id']) {        alert("글이 존재하지 않습니다.\\n글이 삭제되었거나 이동하였을 수 있습니다.".$bo_table);    }
}
...
if ($w == '' || $w == 'r') {
	...
	$sdate = substr($wr_1,0,4)."-".substr($wr_1,4,2)."-".substr($wr_1,6,2);
	$edate = substr($wr_2,0,4)."-".substr($wr_2,4,2)."-".substr($wr_2,6,2);
	$sqls = "select count(*) limitdate from deb_days where date between '$sdate' and '$edate' ";
    //$sqls = "select datediff(date({$wr_2}), date('$wr_1')) as ldate from dual ";
    $limitday = sql_fetch($sqls)['limitdate'];
    $sqls = "select * from deb_days where date between '$sdate' and '$edate' ";
    ...
    for ($i = 0; $rows = sql_fetch_array($results);$i++)  {
	   ...
	   //$csql = "select date_add(date({$wr_1}), interval {$i} day) as day from dual";
       //$scdate = str_replace("-","",sql_fetch($csql)['day']);
		$scdate = str_replace("-","",$rows["date"]);
        //$scdate = str_replace("-","",$rows["date"]);
...
// 파일개수 체크
$upload_count = $file_count   = 0;
if ($_FILES && $_FILES['bf_file']['name'])
...
$file_upload_msg = '';
$upload = array();
$cnt = ($_FILES && $_FILES['bf_file']['name'])?count($_FILES['bf_file']['name']):0;
for ($i=0; $i<$cnt; $i++) {
    $upload[$i]['file']     = '';
    $upload[$i]['source']   = '';
    $upload[$i]['filesize'] = 0;
