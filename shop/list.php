...
@include_once($list_skin_path.'/lrist.head.skin.php');

$order_by = ($sort != "") ? $sort.' '.$sortodr.' ,'.$sql_apms_orderby.' it_order, pt_num desc, it_id desc' : $sql_apms_orderby.' it_order, pt_num desc, it_id desc, ca_id asc'; // 상품 출력순서가 있다면
$where = "it_use = '1' 
		  and a.it_id = (select max(d.it_id) 
			from {$g5['g5_shop_item_table']} d 
			where a.it_id = d.it_id
			and a.it_2 = d.it_2)
...
// 전체 페이지 계산
$row2 = sql_fetch(" select count(*) as cnt from `{$g5['g5_shop_item_table']}` a where $where ");
$total_count = $row2['cnt'];
$total_page  = ceil($total_count / $item_rows);

$num = $total_count - ($page - 1) * $item_rows;

$groupby = " group by it_2 ";
$sql = " select * from `{$g5['g5_shop_item_table']}` a where $where $groupby order by $order_by limit $from_record, $item_rows ";
//echo nl2br($sql)."<BR>";
