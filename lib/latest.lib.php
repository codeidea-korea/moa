...
function g5_latest($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    global $g5;
    ...
    if(!G5_USE_CACHE || $cache_fwrite) {
        ...
        $tmp_write_table = 'g5_' . $bo_table; // 게시판 테이블 전체이름
        $sql = " select * from {$tmp_write_table} order by it_id limit 0, {$rows} ";
        $result = sql_query($sql);
        $bo_subject = get_text($sql['it_name']);
        for ($i=0; $row = sql_fetch_array($result); $i++) {
            try {
                unset($row['wr_password']);     //패스워드 저장 안함( 아예 삭제 )
            } catch (Exception $e) {
            ...
            $list[$i] = get_list($row, $result, $latest_skin_url, $subject_len);
        }
...
//작성자 기준 추출
function latest_writer($skin_dir='', $bo_table, $mb_id, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    global $g5;
    ...
    if(preg_match('#^theme/(.+)$#', $skin_dir, $match)) {
        ...
        $tmp_write_table = $g5['write_prefix'] . $bo_table; // 게시판 테이블 전체이름
        $sql = " select * from {$tmp_write_table} where wr_is_comment = 0 and mb_id='{$mb_id}' order by wr_num limit 0, {$rows}
        ";
        //echo $sql."<BR>";
        $result = sql_query($sql);
        for ($i=0; $row = sql_fetch_array($result); $i++) {
