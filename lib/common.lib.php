...
{
	global $aslang;
    //echo $write_pages." : ".$cur_page." : ".$total_page." : ".$url."<BR>";
...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_list_adm($write_row, $board, $skin_url, $subject_len=40)
{
    global $g5, $config;
    global $qstr, $page;
    ...
    // 분류명 링크
    $list['ca_name_href'] = G5_URL.'/adm/bbs/board.php?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);
    if($list['ca_name']) {
	    $list['ca_name'] = clean_xss_tags($list['ca_name']);
        //$list['ca_name'] = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $list['ca_name']);
	    $list['ca_name'] = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\^\*]/", "", $list['ca_name']);
	}
    $list['href'] = G5_URL.'/adm/bbs/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    $list['comment_href'] = $list['href'];
    ...
    for ($i=1; $i<=G5_LINK_COUNT; $i++) {
        ...
        $list['link'][$i] = set_http(get_text($list["wr_link{$i}"]));
        $list['link_href'][$i] = G5_URL.'/adm/bbs/link.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;no='.$i.$qstr;
        $list['link_hit'][$i] = (int)$list["wr_link{$i}_hit"];
    }
...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_list_host($write_row, $board, $skin_url, $subject_len=40)
{
    global $g5, $config;
    global $qstr, $page;
    ...
    $list['href'] = G5_URL.'/shop/partner/bbs/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    $list['comment_href'] = $list['href'];
    ...
    for ($i=1; $i<=G5_LINK_COUNT; $i++) {
        ...
        $list['link'][$i] = set_http(get_text($list["wr_link{$i}"]));
        $list['link_href'][$i] = G5_URL.'/shop/partner/bbs/link.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;no='.$i.$qstr;
        $list['link_hit'][$i] = (int)$list["wr_link{$i}_hit"];
    }
