...
if($_POST['js'] == "on") {
    ...
    //     $error = '해당 게시물에서만 좋아요 또는 싫어요 하실 수 있습니다.';
    //     print_result($error, $success, $count);
    // }
    ...
    if ($good == 'good' || $good == 'nogood') {
        if($write['mb_id'] == $member['mb_id']) {
            $error = '자신의 글에는 좋아요 또는 싫어요 하실 수 없습니다.';
	        print_result($error, $success, $count);
        }

        if (!$board['bo_use_good'] && $good == 'good') {
            $error = '이 기능은 좋아요 기능을 사용하지 않습니다.';
	        print_result($error, $success, $count);
        }

        if (!$board['bo_use_nogood'] && $good == 'nogood') {
            $error = '이 기능은 싫어요 기능을 사용하지 않습니다.';
	        print_result($error, $success, $count);
        }
        ...
        if ($row['bg_flag']) {
            if ($row['bg_flag'] == 'good')
                $status = ($txt_good) ? $txt_good : '좋아요';
            else
                $status = ($txt_nogood) ? $txt_nogood : '싫어요';
    ...
    // $ss_name = 'ss_view_'.$bo_table.'_'.$wr_id;
    // if (!get_session($ss_name))
    //     alert('해당 게시물에서만 좋아요 또는 싫어요 하실 수 있습니다.');
    ...
    if ($good == 'good' || $good == 'nogood') {
        if($write['mb_id'] == $member['mb_id'])
            alert('자신의 글에는 좋아요 또는 싫어요 하실 수 없습니다.');

        if (!$board['bo_use_good'] && $good == 'good')
            alert('이 기능은 좋아요 기능을 사용하지 않습니다.');

        if (!$board['bo_use_nogood'] && $good == 'nogood')
            alert('이 기능은 싫어요 기능을 사용하지 않습니다.');
