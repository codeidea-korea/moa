...
//echo $_SERVER['HTTP_REFERER']."<BR>"; exit;
if (strrpos($_SERVER['HTTP_REFERER'],"/shop/partner")!==false)
   $w = "u";
if ($w == "" ) {
    ...
    //if ($is_admin == 'super')        alert('관리자의 회원정보는 관리자 화면에서 수정해 주십시오.', G5_URL);

    if (!$is_member)
        alert('로그인 후 이용하여 주십시오.', G5_URL);

    $ssmb = get_session('ss_mb_id');
    //if ($member['mb_id'] != $_POST['mb_id'])
    if ($member['mb_id'] != $ssmb)
