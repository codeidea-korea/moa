...
$query1 = "select si.*, wc.as_thumb from g5_shop_item as si join g5_write_class as wc on si.it_2 = wc.wr_id ".$joinQuery." where wc.moa_status = 1 and wc.moa_pick = '기획전' ".$whereQuery." group by wc.wr_id order by wc.wr_hit desc";
...
$query2 = "select si.*, wc.as_thumb from g5_shop_item as si join g5_write_class as wc on si.it_2 = wc.wr_id ".$joinQuery." where wc.moa_status = 1 and wc.moa_pick = '모아픽' ".$whereQuery." group by wc.wr_id order by wc.wr_hit desc";
...
$sql1 = "select *, count(player.it_id) as cnt from g5_write_class wc join g5_shop_item si on wc.wr_id = si.it_2 ".$joinQuery." join deb_class_aplyer player on si.it_id = player.it_id where 1=1 ".$whereQuery." group by player.it_id having count(player.it_id) > 0 and wc.moa_status = 1";
...
$query4 = "select si.*, wc.as_thumb, wc.moa_area1 from g5_shop_item as si join g5_write_class as wc on si.it_2 = wc.wr_id ".$joinQuery." where 1=1 ".$whereQuery." order by si.it_hit where wc.moa_status = 1 desc limit 0, 10";
...
$query5 = "select iu.*,wc.as_thumb from g5_shop_item_use as iu join g5_shop_item as si on iu.it_id = si.it_id join g5_write_class wc on si.it_2 = wc.wr_id ".$joinQuery." 
