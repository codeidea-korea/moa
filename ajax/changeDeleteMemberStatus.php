...
//                get_member_refund_order($order['mb_id'], $type='host', $ment='부분 환불', $order['od_id']);
            ...
            */
            ...
            $sql = "update g5_write_class set moa_status = 2, moa_close_time = '{$close_time}', moa_close_reason = '호스트 회원 탈퇴에 의한 반려' 
                ...
                */
            $sql = "update g5_write_class set moa_status = 4, moa_close_time = '{$close_time}', moa_close_reason = '호스트 회원 탈퇴에 의한 취소' 
                where wr_id in (".$moim_ids.") and (moa_status = 1 or moa_status = 0) ";
            sql_fetch($sql);
            /*
        } else {
